...
const getCatdata = async (catname) => {
  try {
    const category = await axios.get(
      `${process.env.BASE_URL}/api/blog?search=${catname}`
    );
...
const page = async (props) => {
  const data = await getCatdata(props.searchParams.search);
  const posts = data["category"].data;
      ...
      <Plainlayout>
        <div className="container mx-auto mt-5">
          <h2 className="text-4xl font-semibold ">
            Search Text :{" "}
            <span className="text-maincolor">{props.searchParams.search}</span>
          </h2>
          {posts.length === 0 && (
